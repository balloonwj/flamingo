#include "stdafx.h"
#include "BuddyListCtrlEx.h"

CBuddyItem::CBuddyItem(void)
{
	m_nID = 0;
	m_bShowNickName = TRUE;
...
void CBuddyListCtrl::SetBuddyItemNickName(int nTeamIndex, int nIndex, LPCTSTR lpszText, BOOL bShow)
{
	CBuddyItem * lpItem = GetBuddyItemByIndex(nTeamIndex, nIndex);
	if (lpItem != NULL)
	{
		lpItem->m_strNickName = lpszText;
		lpItem->m_bShowNickName = bShow;
	}
}

void CBuddyListCtrl::SetBuddyItemAccount(int nTeamIndex, int nIndex, LPCTSTR lpszText, BOOL bShow)
{
	CBuddyItem * lpItem = GetBuddyItemByIndex(nTeamIndex, nIndex);
	if (lpItem != NULL)
	{
...
void CBuddyListCtrl::SetBuddyItemDate(int nTeamIndex, int nIndex, LPCTSTR lpszText)
{
	CBuddyItem* lpItem = GetBuddyItemByIndex(nTeamIndex, nIndex);
	if (lpItem != NULL)
...
void CBuddyListCtrl::SetBuddyItemSign(int nTeamIndex, int nIndex, LPCTSTR lpszText, BOOL bSignature/*=TRUE*/)
{
	CBuddyItem * lpItem = GetBuddyItemByIndex(nTeamIndex, nIndex);
	if (lpItem != NULL)
	{
		lpItem->m_strSign = lpszText;
		lpItem->m_bSignature = bSignature;
	}
}
...
BOOL CBuddyListCtrl::SetBuddyItemMobilePic(int nTeamIndex, int nIndex, LPCTSTR lpszFileName)
{
	CBuddyItem* lpItem = GetBuddyItemByIndex(nTeamIndex, nIndex);
	if (NULL == lpItem)
		return FALSE;
...
CString CBuddyListCtrl::GetBuddyItemAccount(int nTeamIndex, int nIndex)
{
	CBuddyItem * lpItem = GetBuddyItemByIndex(nTeamIndex, nIndex);
	if (lpItem != NULL)
...
void CBuddyListCtrl::DrawBuddyItemInBigIcon(HDC hDC, int nTeamIndex, int nIndex)
{
	...
	CString strName1(lpItem->m_strNickName);
	CString strName2;
	if(lpItem->m_bShowNickName && lpItem->m_bShowAccountName && lpItem->m_bSignature)
	{
		strName2.Format(_T("(%s)"), lpItem->m_strAccount);
	}
	else
		strName2 = lpItem->m_strAccount;
	...
	CRect rcSign(0, 0, 0, 0);
	CRect rcDate(0, 0, 0, 0);
	
	//TODO: 这段代码需要重写！！！
	if (lpItem->m_bShowNickName && strName1.GetLength()>0 && !lpItem->m_bShowAccountName)
	{
	...
	else if(lpItem->m_bShowAccountName && strName2.GetLength()>0 && !lpItem->m_bShowNickName)
	{
		CRect rcText(0,0,0,0);	// 计算文字宽高
		::DrawText(hDC, strName2, strName2.GetLength(), &rcText, DT_SINGLELINE | DT_CALCRECT);
		...
		if (lpItem->m_bOnlineAnim)
		{
			...
			clrText = RGB(0, 0, 0);
		::SetTextColor(hDC, clrText);
		::DrawText(hDC, strName2, strName2.GetLength(), &rcName1, nFormat);
	}
	else 
	{	
		CRect rcText(0,0,0,0);	// 计算文字宽高
		...
		if (lpItem->m_bShowAccountName && strName2.GetLength() > 0)
		{
			CRect rcText(0,0,0,0);	// 计算文字宽高
			::DrawText(hDC, strName2, strName2.GetLength(), &rcText, DT_SINGLELINE | DT_CALCRECT);
	...
	if(lpItem->m_strDate.GetLength() > 0)
	{
		CRect rcText(0,0,0,0);	// 计算文字宽高
		::DrawText(hDC, lpItem->m_strDate, lpItem->m_strDate.GetLength(), &rcText, DT_SINGLELINE | DT_CALCRECT);
		
		rcDate.left = rcItem.right-10-rcText.Width();	
		rcDate.right = rcItem.right-10; 
		rcDate.top = rcItem.top + 9;
		rcDate.bottom = rcDate.top + rcText.Height();
...
void CBuddyListCtrl::DrawBuddyItemInSmallIcon(HDC hDC, int nTeamIndex, int nIndex)
{
	...
	//小头像只显示昵称
	CString strName(lpItem->m_strNickName);
