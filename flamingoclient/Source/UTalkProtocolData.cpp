#include "stdafx.h"
#include "UTalkProtocolData.h"
#include "webcommunication/IUProtocolData.h"
#include "Utils.h"
#include "Path.h"
#include "Buffer.h"
#include "webcommunication/protocol.h"
...
{
	if (m_nType != UTalk_MSG_TYPE_BUDDY)
		return NULL;
...
{
	if (m_nType != UTalk_MSG_TYPE_GROUP)
		return NULL;
...
{
	if (m_nType != UTalk_MSG_TYPE_SESS)
		return NULL;
...
{
	//if (m_nType != UTalk_MSG_TYPE_SYSGROUP)
	//	return NULL;
...
CBuddyMessage * CMessageList::GetBuddyMsg(UINT nMsgId)
{
	...
	{
		CMessageSender * lpMsgSender = m_arrMsgSender[i];
		if (lpMsgSender != NULL && lpMsgSender->GetMsgType() == UTalk_MSG_TYPE_BUDDY)
		{
...
CBuddyMessage * CMessageList::GetGroupMsg(UINT nMsgId)
{
	...
	{
		CMessageSender * lpMsgSender = m_arrMsgSender[i];
		if (lpMsgSender != NULL && lpMsgSender->GetMsgType() == UTalk_MSG_TYPE_GROUP)
		{
...
CSessMessage * CMessageList::GetSessMsg(UINT nMsgId)
{
	...
	{
		CMessageSender * lpMsgSender = m_arrMsgSender[i];
		if (lpMsgSender != NULL && lpMsgSender->GetMsgType() == UTalk_MSG_TYPE_SESS)
		{
...
{
	CMessageSender * lpMsgSender = GetMsgSender(UTalk_MSG_TYPE_BUDDY, nUTalkUin);
	if (lpMsgSender != NULL)
...
{
	CMessageSender * lpMsgSender = GetMsgSender(UTalk_MSG_TYPE_GROUP, nGroupCode);
	if (lpMsgSender != NULL)
...
{
	CMessageSender * lpMsgSender = GetMsgSender(UTalk_MSG_TYPE_SESS, nUTalkUin);
	if (lpMsgSender != NULL)
